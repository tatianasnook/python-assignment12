...
# Setting a seed so results are consistent every time you run
np.random.seed(42)
sample_data = {
...
selected_product = st.sidebar.selectbox(
    'Select Product', df['Product'])  # Dropdown to choose a product

# Filter the data based on the user's selection
# Show only the row that matches the selected product
...
with col1:
    # Show sales in a pretty format
    st.metric('Sales', f"${filtered_df['Sales'].values[0]:,}")
with col2:
    # Show profit similarly
